...
int main(int argc, char **argv)
{
    ...
    if(rank == 0){
        ...
        double pi_estimate_mach = 16*global_sum[0] - 4*global_sum[1];
        double duration = MPI_Wtime() - time_start;

        printf("Calcualted value of pi with Mach method on %d processors: %.17g\n", size, pi_estimate_mach);
        printf("The error with %d processors and %d steps was: %.17g\n", size, n, fabs(pi-pi_estimate_mach));
